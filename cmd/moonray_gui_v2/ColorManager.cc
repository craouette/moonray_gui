...
/// TODO: Are the doubles used in this file necessary?  Would floats suffice?
/// Or does OCIO require doubles?
...
void setHotChannel(const DebugMode mode, std::array<int, 4>& channelHot) 
{
    switch (mode) {
    case RGB:
        channelHot = {1, 1, 1, 1};
...
void floatBufferToRgb888(const float* src, const int w, const int h, const int channels, Rgb888Buffer* dst) 
{
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x) {
            ByteColor col8;
